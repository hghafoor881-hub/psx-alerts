...
def get_all_symbols():
    try:
        all_symbols = list(psxdata.tickers())
        # Sirf asal stocks rakhein — bonds/T-bills/TFCs ke naam mein numbers hote hain
        # (jaise PK03TB020426, P10PFL031034), equity symbols sirf letters ke hote hain
        equity_only = [s for s in all_symbols if s.isalpha()]
        return equity_only if equity_only else all_symbols
    except Exception:
